...
// SedScript is a GNU-style substitution script of the form
// s<delim>pattern<delim>replacement<delim>flags, e.g. "s/old/new/g".
type SedScript string

// patternText is the raw regular-expression part of an s/// script.
type patternText string

// flagText is the raw trailing-flag part of an s/// script.
type flagText string

// flagRune is a single character of an s/// flag string.
type flagRune rune

// ignoreCaseFlag selects case-insensitive pattern matching (the i flag).
...
//
//   - globalEnabled:     g — replace every match on the line, not just the first.
//   - ignoreCaseEnabled: i — match case-insensitively.
//   - printIfSubEnabled: p — emit the line a second time when a substitution occurred.
//   - nth:               N — replace only the Nth match (1-based); 0 means "first".
type substFlags struct {
	globalEnabled     bool
	ignoreCaseEnabled bool
	printIfSubEnabled bool
	nth               int
}
...
func compile(script SedScript) (substitution, error) {
	parsed, err := split(script)
	if err != nil {
		return substitution{}, err
	}
	flags, err := parseFlags(flagText(parsed.flagString))
	if err != nil {
		return substitution{}, err
	}
	re, err := compilePattern(patternText(parsed.pattern), ignoreCaseFlag(flags.ignoreCaseEnabled))
	if err != nil {
		return substitution{}, err
	}
...
func split(script SedScript) (expression, error) {
	text := string(script)
	if len(text) < 2 || text[0] != 's' {
		return expression{}, ErrUnsupportedExpression
	}
	parts := strings.SplitN(text[2:], text[1:2], 3)
	if len(parts) < 3 {
...
func compilePattern(pattern patternText, isIgnoreCase ignoreCaseFlag) (*regexp.Regexp, error) {
...
func parseFlags(text flagText) (substFlags, error) {
	flags := substFlags{}
	for _, r := range string(text) {
		next, err := applyFlagRune(flagRune(r), flags)
		if err != nil {
...
func applyFlagRune(r flagRune, flags substFlags) (substFlags, error) {
	switch rune(r) {
	case 'g':
		flags.globalEnabled = true
	case 'i':
		flags.ignoreCaseEnabled = true
	case 'p':
		flags.printIfSubEnabled = true
	default:
...
func applyDigit(r flagRune, flags substFlags) (substFlags, error) {
	if r < '0' || r > '9' {
		return substFlags{}, ErrInvalidFlags
	}
...
func (s substitution) apply(line []byte) ([][]byte, error) {
	...
	if s.flags.printIfSubEnabled && substituted {
